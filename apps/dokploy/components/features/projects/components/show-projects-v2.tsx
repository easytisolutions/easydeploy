...
import { StatusPulse, EmptyProjectsIllustration } from "@/components/atoms/animations/animated-svgs";
import {
	PageTransition,
	StaggerList,
	StaggerItem,
	...
	const filteredProjects = useMemo(() => {
		...
		if (selectedTagIds.length > 0) {
			filtered = filtered.filter((project) =>
				project.projectTags?.some((pt) => selectedTagIds.includes(pt.tag.tagId)),
			);
		}
		...
		return [...filtered].sort((a, b) => {
			...
			switch (field) {
				...
				case "createdAt":
					...
					break;
				case "services": {
					const aTotal = a.environments.reduce(
						(total, env) =>
							total + (env.applications?.length || 0) + (env.mariadb?.length || 0) + (env.mongo?.length || 0) + (env.mysql?.length || 0) + (env.postgres?.length || 0) + (env.redis?.length || 0) + (env.compose?.length || 0),
						0,
					);
					const bTotal = b.environments.reduce(
						(total, env) =>
							total + (env.applications?.length || 0) + (env.mariadb?.length || 0) + (env.mongo?.length || 0) + (env.mysql?.length || 0) + (env.postgres?.length || 0) + (env.redis?.length || 0) + (env.compose?.length || 0),
						0,
					);
							...
							{isPending ? (
								<div className="flex flex-col gap-4 items-center justify-center min-h-[60vh]">
									<motion.div
										...
										className="w-8 h-8 border-3 border-easyti-primary/30 border-t-easyti-primary rounded-full"
									/>
								...
								<>
									...
									<div className="flex max-sm:flex-col gap-4 items-center w-full">
										...
										<div className="flex items-center gap-2">
											<TagFilter
												tags={availableTags?.map((tag) => ({ id: tag.tagId, name: tag.name, color: tag.color || undefined })) || []}
											...
											<div className="flex items-center gap-2 min-w-48 max-sm:w-full">
												...
												<Select value={sortBy} onValueChange={setSortBy}>
													...
													<SelectContent>
														...
														<SelectItem value="services-desc">Mais serviços</SelectItem>
														<SelectItem value="services-asc">Menos serviços</SelectItem>
													</SelectContent>
												</Select>
											</div>
										</div>
									...
									{filteredProjects?.length === 0 && (
										<motion.div
											...
											className="mt-6 flex h-[50vh] w-full flex-col items-center justify-center space-y-4"
										>
											...
											{!debouncedSearchQuery && permissions?.project.create && (
												<Button
									...
									{filteredProjects && filteredProjects.length > 0 && (
										<motion.div
											...
											initial="hidden"
											animate="visible"
											className="w-full grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 3xl:grid-cols-5 flex-wrap gap-5"
										>
											{filteredProjects.map((project) => {
												...
												return (
													...
													>
														...
														>
															<Card className="group relative w-full h-full bg-transparent transition-all duration-200 hover:bg-border hover:shadow-md hover:-translate-y-0.5">
																<CardHeader>
																	<CardTitle className="flex items-center justify-between gap-2 overflow-clip">
																		<span className="flex flex-col gap-1.5">
																			<div className="flex items-center gap-2">
																				<div className="w-8 h-8 rounded-lg bg-easyti-primary/10 flex items-center justify-center">
																					<BookIcon className="size-4 text-easyti-primary" />
																				</div>
																			...
																			{project.description && (
																				<span className="text-sm font-normal text-muted-foreground break-normal ml-10">
																					{project.description}
																				</span>
																			)}
																		...
																		<div className="flex self-start space-x-1">
																			<DropdownMenu>
																				<DropdownMenuTrigger asChild>
																					<Button variant="ghost" size="icon" className="px-2 opacity-0 group-hover:opacity-100 transition-opacity">
